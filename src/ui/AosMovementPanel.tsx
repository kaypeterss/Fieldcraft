...
export function AosMovementPanel(props: AosMovementPanelProps) {
  ...
  const allowance = actionId === 'RUN' ? baseMove + (rollResult ?? 0) : baseMove
  return <aside className="movement-panel aos-movement-panel" aria-label="Age of Sigmar movement">
    <div className="movement-panel-heading">
      <div><span className="eyebrow">MOVEMENT</span><strong>{props.unitName}</strong></div>
    ...
    <dl>
      ...
      {rollResult !== undefined && <div><dt>{actionId === 'RUN' ? 'Run Roll' : 'Retreat Damage'}</dt>
        <dd>{actionId === 'RUN' ? `+${rollResult}″` : `D3 = ${rollResult}`}</dd></div>}
