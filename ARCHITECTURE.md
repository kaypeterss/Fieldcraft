...
`src/engine/spatial.ts` is the game-agnostic spatial-query boundary. It operates only in tabletop inches on domain models and points, and delegates circular base-edge calculations to the existing geometry primitive. The measurement tool uses this same high-level geometry, preventing divergent distance definitions. Current queries cover model-to-model and model-to-point distance with closest anchors, model/unit range membership, unit-to-unit and unit-to-point minimum distance with the responsible model or model pair, and target-center exclusion radii.

`src/tools/measurement.ts` resolves all ordered combinations of point, model, and unit targets against live state. Point-to-point uses Euclidean distance. A circular model measures from its base edge, and a unit measures from whichever current member yields the minimum base-edge distance. Results preserve target order, return `startAnchor` and `endAnchor` in tabletop coordinates, and identify responsible models where applicable. Touching, overlapping, point-inside-base, and coincident-center cases return zero with deterministic coincident anchors; missing models and empty or unresolvable units return no result. Ownership and turn state do not alter these spatial facts.
